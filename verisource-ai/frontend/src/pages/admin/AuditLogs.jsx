...
const AuditLogs = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            accessor: 'decision',
            cell: (row) => (
                <span className={`text-xs font-bold flex items-center gap-1.5 ${row.decision === 'Approved' ? 'text-green-400' : 'text-red-400'
                    }`}>
                    <span className={`w-1.5 h-1.5 rounded-full ${row.decision === 'Approved' ? 'bg-green-400' : 'bg-red-400'}`}></span>
                    {row.decision}
                </span>
            )
        },
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-2">
                ...
                <button className="flex items-center gap-2 px-4 py-2 bg-brand-navy border border-white/10 hover:bg-white/5 rounded-lg text-sm transition-colors text-white">
